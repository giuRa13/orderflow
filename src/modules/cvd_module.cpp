...
/*void CVDModule::render(MarketData& data)
{
    if (!is_open) return;

    if (data.candles.empty()) 
    {
        ImGui::Begin("Cumulative Volume Delta", &is_open);
    ...
    double min_c = data.candles[0].cvd_low;
    double max_c = data.candles[0].cvd_high;
    for (auto& c : data.candles) 
    {
        min_c = std::min(min_c, c.cvd_low);
        max_c = std::max(max_c, c.cvd_high);
    }
    ...
    if (ImPlot::BeginPlot("##CVDPlot", ImVec2(-1, -1)))
    {
        ImPlot::SetupAxisScale(ImAxis_X1, ImPlotScale_Time);
        ...
        CommonRender::plot_candlesticks("CVD", data.candles.data(), (int)data.candles.size(), data.tick_timeframe * 0.8, true, data.ask_color, data.bid_color);
        
        double current_cvd = data.candles.back().cvd_close;
        ImVec4 tag_color = (data.candles.back().cvd_close >= data.candles.back().cvd_open) 
            ? data.ask_color   
            : data.bid_color;
        CommonRender::draw_price_line(current_cvd, tag_color);
